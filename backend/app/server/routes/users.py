from fastapi import APIRouter, Header, Body, HTTPException, status
...
async def register_user_choice(user_id: str, user_choice: Choice = Body()):
    def add_choice_to_user(user: User):
        if user.id == user_id:
            if user_choice in user.choices:
                raise HTTPException(
